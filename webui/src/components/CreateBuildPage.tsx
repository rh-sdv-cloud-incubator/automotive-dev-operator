...
const CreateBuildPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let t: BuildTemplateResponse | null = (location as any)?.state?.template || null;
    if (!t) {
    ...
    setFormData(prev => ({
      ...
      automotiveImageBuilder: t?.automotiveImageBuilder ?? prev.automotiveImageBuilder,
    ...
    if (t.sourceFiles && t.sourceFiles.length > 0) {
      setTextFiles(prev => {
        ...
        const toAdd = t!.sourceFiles!
          ...
          .map(p => ({ id: `text-${crypto.randomUUID?.() || Date.now()}`, name: p, content: '' } as TextFile));
        return [...prev, ...toAdd];
      });
    }
  ...
  const uploadFiles = async (buildName: string) => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const response = await fetch(`/v1/builds/${buildName}/uploads`, {
          method: 'POST',
          body: formData,
        });
        ...
        if (attempt >= maxAttempts) {
          ...
          setAlert({ type: 'danger', message: `Error uploading files: ${error}` });
          return;
        }
        await sleep(delayMs);
      }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        aibExtraArgs: formData.aibExtraArgs ? formData.aibExtraArgs.split(' ').filter(arg => arg.trim()) : [],
        aibOverrideArgs: formData.aibOverrideArgs ? formData.aibOverrideArgs.split(' ').filter(arg => arg.trim()) : [],
      ...
      if (response.ok) {
        ...
        setAlert({ type: 'success', message: `Build "${result.name}" created successfully!` });
        ...
        <CardBody>
          <Form onSubmit={handleSubmit}>
            <Grid hasGutter>
              ...
              <GridItem span={6}>
                <FormGroup label="Manifest File Name" fieldId="manifestFileName">
                  <TextInput
                    id="manifestFileName"
              ...
              <GridItem span={12}>
                <FormGroup label="Manifest Content" isRequired fieldId="manifest">
                  <TextArea
                    id="manifest"
              ...
              <GridItem span={4}>
                <FormGroup label="Distribution" fieldId="distro">
                  <TextInput
                    ...
                    onChange={(_event, value) => handleInputChange('distro', value)}
                    placeholder="cs9"
                  />
                </FormGroup>
              </GridItem>
              ...
              <GridItem span={4}>
                <FormGroup label="Target" fieldId="target">
                  <TextInput
                    ...
                    onChange={(_event, value) => handleInputChange('target', value)}
                    placeholder="qemu"
                  />
                </FormGroup>
              </GridItem>
              ...
              <GridItem span={4}>
                <FormGroup label="Architecture" fieldId="architecture">
                  <TextInput
                    ...
                    onChange={(_event, value) => handleInputChange('architecture', value)}
                    placeholder="arm64"
                  />
                </FormGroup>
              </GridItem>
              ...
              <GridItem span={6}>
                <FormGroup label="Export Format" fieldId="exportFormat">
                  <TextInput
                    ...
                    onChange={(_event, value) => handleInputChange('exportFormat', value)}
                    placeholder="image"
                  />
                </FormGroup>
              </GridItem>
              ...
              <GridItem span={6}>
                <FormGroup label="Mode" fieldId="mode">
                  <TextInput
                    id="mode"
                    value={formData.mode}
                    onChange={(_event, value) => handleInputChange('mode', value)}
                    placeholder="image"
                  />
                </FormGroup>
              </GridItem>
              ...
              <GridItem span={12}>
                <FormGroup label="Automotive Image Builder Container" fieldId="automotiveImageBuilder">
                  <TextInput
              ...
              <GridItem span={12}>
                ...
                >
                  <Grid hasGutter>
                    <GridItem span={6}>
                      <FormGroup label="AIB Extra Arguments" fieldId="aibExtraArgs">
                        <TextInput
                          id="aibExtraArgs"
                    ...
                    <GridItem span={6}>
                      <FormGroup label="AIB Override Arguments" fieldId="aibOverrideArgs">
                        <TextInput
                          id="aibOverrideArgs"
            ...
            <ExpandableSection toggleText="File Management" isExpanded>
              <Tabs activeKey={activeFileTab} onSelect={(_event, tabIndex) => setActiveFileTab(tabIndex)}>
                <Tab eventKey={0} title={<TabTitleText>Text Files</TabTitleText>}>
                  <div style={{ padding: '16px 0' }}>
                    ...
                    {textFiles.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '20px', color: '#6A6E73' }}>
                        No text files added. Click "Add Text File" to create a new file.
                      </div>
                    ) : (
                      <List>
                        {textFiles.map((file) => (
                ...
                <Tab eventKey={1} title={<TabTitleText>File Uploads</TabTitleText>}>
                  <div style={{ padding: '16px 0' }}>
                    ...
                    {uploadedFiles.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '20px', color: '#6A6E73' }}>
                        No files uploaded. Use the file browser above to select files.
                      </div>
                    ) : (
